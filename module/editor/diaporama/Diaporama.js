...
Class.create("Diaporama", View, {
	...
	initialize : function($super, oFormObject, options)
	{
        ...
        var ipConfigs = application.getGuiComponentConfigs("InfoPanel");
		...
		if(application && application.user){
			var autoFit = application.user.getPreference('diapo_autofit');
	...
	toggleFitToScreen : function(skipSave){
		...
		if(application && application.user && !skipSave){
			application.user.setPreference("diapo_autofit", (this.autoFit ? 'true' : 'false'));
			application.user.savePreferences();
		}
	},
		...
		img.observe("mouseover", function(event){
			...
			if(!theImage.openBehaviour){
				...
				theImage.observe("click", function(event){
					application.actionBar.fireAction('open_with');
				});
			}
