...
Class.create("Action", {
	...
	apply : function(){
		if(this.deny) return;
        document.fire("application:beforeApply-"+this.options.name);
		...
		if(this.options.callbackCode) {
			try{
				...
				application.displayMessage('ERROR', e.message);
			}
        ...
        document.fire("application:afterApply-"+this.options.name);
	},
