...
Class.create("ViewManager", {
	initialize : function()
	{
		...
		document.observeOnce("application:component_config_changed", function(event){
			$$(".view").each(function(element){
				...
				var editors = application.findEditorsForMime(mime);
	...
	_createEditor : function(element, data){
		if(data){
			application.loadEditorResources(data.resourcesManager);
			if(!data.formId){
