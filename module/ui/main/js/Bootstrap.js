...
Class.create("Bootstrap", {
	...
	refreshContextVariablesAndInit : function(connexion){
		...
		if(this.parameters.get('additional_js_resource')){
			connexion.loadLibrary(this.parameters.get('additional_js_resource?v='+this.parameters.get("version")));
		}
		this.insertLoaderProgress();
		if(!this.parameters.get("debug")){
			connexion.loadLibrary("app.js?v="+this.parameters.get("version"));
		}
	...
	_onSettingsLoaded : function(transport){
		...
		if(this.parameters.get('SERVER_PREFIX_URI')){
			this.parameters.set('serverAccess', this.parameters.get('SERVER_PREFIX_URI') + this.parameters.get('serverAccess') + '?' + (Connexion.SECURE_TOKEN ? 'secure_token=' + Connexion.SECURE_TOKEN : ''));
		}else{
			this.parameters.set('serverAccess', this.parameters.get('serverAccess') + '?' + (Connexion.SECURE_TOKEN ? 'secure_token=' + Connexion.SECURE_TOKEN : ''));
		}
