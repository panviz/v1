/**
 * A selector for displaying repository list. Will hook to application:repository_list_refreshed.
 */
Class.create("RepositorySelect", {
	...
	initialize : function(oElement){
		...
		this.show = true;
		this.createGui();
		document.observe("application:repository_list_refreshed", function(e){
	...
	refreshRepositoriesMenu : function(repositoryList, repositoryId){
        ...
        var menuItems = $A();
        var otherActions = application.actionBar.getActionsForWidget("RepositorySelect", this.element.id).each(function(otherAction){
            menuItems.push({
	...
	onRepoSelect : function(key){
		application.triggerRepositoryChange(key);
	},
	/**
	 * Resize widget
	 */
