...
function Tree(rootItem, sAction, filter) {
	this.WebFXTree = WebFXTree;
	this.loaded = true;
	this.item = rootItem;
	var icon = rootItem.getIcon();
	if(icon.indexOf(THEME.path+"/") != 0){
		icon = resolveImageSource(icon, "/image/mime/ICON_SIZE", 16);
	}
	var openIcon = rootItem.getMetadata().get("openicon");
	if(openIcon){
	...
	this.WebFXTree(rootItem.getLabel(), sAction, 'explorer', icon, openIcon);
    ...
    this.overlayIcon = splitOverlayIcons(rootItem);
...
Tree.prototype.setRootItem = function(rootItem){
	if(this.item){
	...
	this.item.observe("item_replaced",  clear.bind(this));
	this.attachListeners(this, rootItem);
	if(oldItem){
		oldItem.notify("item_replaced");
	}
	//this.item.load();
};
...
Tree.prototype.attachListeners = function(jsNode, item){
	...
	item.observe("item_replaced", function(newNode){
